...
Bulk-build mode uses a `BottleneckController` (`crates/indexer/src/sync/bottleneck.rs`) with two independent dimensions: (1) batch sizing via overlap-scaled cell targeting — `target_cells` grows toward `TARGET_ITERATION_MS` (3s) dampened by pipeline overlap ratio (`build / (build + waste)`), shrinks at full strength; (2) I/O resources governed by waste classification (recv wait vs flush wait) adjusting `fetch_threads` and `bg_jobs`. Drain uses `drain_by_cells(target_cells, max_batch_bytes)` with cell count as primary budget and RAM-derived bytes as safety cap. Channel depths (prefetch + flush) are derived from system RAM.
