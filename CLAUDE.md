...
- **Domain store responsibility**: domain store (`[store].domain_data_path`, 53 CFs) holds all mutable canonical/query state including activities, addr_txs, live/consumed cell markers, and all indexes. May perform create/update/delete as required by chain progression and reorg handling, but only via indexer.
...
- `CF_CELLS` is the only append-only CF. All other CFs (53) are domain (canonical mutable view).
...
docs/STORE_SCHEMA.md         # Column families reference (53 domain + 1 append-only)
