...
Bulk-build mode uses a `BottleneckController` (`crates/indexer/src/sync/bottleneck.rs`) with two independent dimensions: (1) batch sizing via wall-clock band + build/IO overlap — primary objective is wall clock in [1s, 3s]; below band → grow, above band → shrink; in-band the controller grows while build > IO (IO headroom exists), holds when IO ≥ build (physical limit); (2) I/O resources governed by waste classification (recv wait vs flush wait) adjusting `fetch_threads` and `bg_jobs`. Drain uses `drain_by_cells(target_cells, max_batch_bytes)` with cell count as primary budget and RAM-derived bytes as safety cap. Prefetch fill estimate uses actual cell density from the buffer (`cell_density()`) instead of hardcoded assumptions. Channel depths (prefetch + flush) are derived from system RAM.
