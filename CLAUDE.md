...
- `CF_CELLS` is the only append-only CF. All other CFs (59) are domain (canonical mutable view).
...
crates/
  ...
  ckbadger-store/ # Embedded RocksDB storage engine (dual-store, 60 domain + 1 append-only CFs)
...
Bulk-build mode uses a `BottleneckController` (`crates/indexer/src/sync/bottleneck.rs`) with two independent dimensions: (1) batch sizing via overlap-driven cell targeting — single objective is pipeline overlap ≥ 90%; when overlap is below target, waste composition determines direction (recv-dominated → grow, flush-dominated → shrink, mixed → geometric blend ≈ hold); when overlap is above target, grow proportional to headroom for overhead amortization; supply cap limits `target_cells` to 2× actual delivered cells to prevent runaway, but only when overlap is below target (genuinely supply-limited); when overlap is healthy, low actual_cells reflects sparse chain data, not a bottleneck, so the cap is skipped to avoid a death spiral in low-density regions; (2) I/O resources governed by waste classification (recv wait vs flush wait) adjusting `fetch_threads` and `bg_jobs`. Drain uses `drain_by_cells(target_cells, max_batch_bytes)` with cell count as primary budget and RAM-derived bytes as safety cap. Prefetch fill estimate uses actual cell density from the buffer (`cell_density()`) instead of hardcoded assumptions. Channel depths (prefetch + flush) are derived from system RAM.
