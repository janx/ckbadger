...
- **Domain store responsibility**: domain store (`[store].domain_data_path`, 43 CFs) holds all mutable canonical/query state including activities, addr_txs, live/consumed cell markers, and all indexes. May perform create/update/delete as required by chain progression and reorg handling, but only via indexer.
...
- `CF_CELLS` is the only append-only CF. All other CFs (43) are domain (canonical mutable view).
...
crates/
  ...
  ckbadger-store/ # Embedded RocksDB storage engine (dual-store, 43 domain + 1 append-only CFs)
...
docs/STORE_SCHEMA.md         # Column families reference (43 domain + 1 append-only)
