...
| `cell_script_versions`           | out_point                                             | CellScriptVersionInfo   | Per-cell script attribution at cell creation time; output type scripts resolve versions from tx deps, while output lock `hash_type=type` rows may carry no version because creation does not execute the lock and dead refs are protocol-valid |
...
- exact version attribution for historical execution must come from the transaction's actual
  `cell_deps`
- created output lock refs with `hash_type=type` may legitimately have no version attribution at
  creation time because the lock is not executed and dead refs are protocol-valid
