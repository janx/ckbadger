# ckbadger-store Column Families (43 total)
...
| --------------------------- | ----------------------------------------------------- | ----------------------- | ----------------------------------------------------------- |
| `cells`                     | tx_hash + output_index (34B)                          | LiveCellInfo            | Legacy cell payload CF kept during schema migration         |
| `cell_state`                | tx_hash + output_index (34B)                          | CellState               | Canonical mutable cell state + payload pointer              |
| `live_cells`                | tx_hash + output_index (34B)                          | empty                   | Legacy live marker CF kept during schema migration          |
| `consumed_cells`            | tx_hash + output_index (34B)                          | ConsumedCellMeta        | Legacy consumed marker CF kept during schema migration      |
...
| `cell_by_lock`              | lock_script_hash + outpoint                           | empty                   | Legacy lock index CF kept during schema migration           |
| `cell_by_type`              | type_script_hash + outpoint                           | empty                   | Legacy type index CF kept during schema migration           |
| `cell_by_lock_code`         | lock_code_hash + outpoint                             | empty                   | Legacy lock code index CF kept during schema migration      |
| `cell_by_type_code`         | type_code_hash + outpoint                             | empty                   | Legacy type code index CF kept during schema migration      |
...
| `cell_payloads`             | created_block + tx_hash + output_index                | LiveCellInfo            | Append-only canonical cell payload history                  |
| `cell_index`                | tag + hash + block_desc + outpoint                    | empty                   | Append-only unified cell history index                      |
| `activity_tx_envelopes`     | block_desc (8B BE) + tx_index (4B BE) + tx_hash (32B) | ActivityTxEnvelope      | Append-only tx-level activity envelope                      |
| `activity_by_owner`         | lock_hash + block_desc + tx_index + tx_hash           | owner_slot (u16)        | Append-only owner -> envelope projection index              |
