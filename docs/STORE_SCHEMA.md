# ckbadger-store Column Families (53 total: 52 domain + 1 append-only)

ckbadger runs two logical RocksDB stores (both backed by `ckbadger-store`):

- **Domain store** (`[store].domain_data_path`, 52 CFs) — canonical chain view, all mutable state including activities, addr_txs, live/consumed cell markers, indexes, stats, and aggregates. May perform create/update/delete as required by chain progression and reorg handling.
