...
pub struct App {
    ...
    us_per_cell_build_history: VecDeque<f64>,
    us_per_cell_flush_history: VecDeque<f64>,
    target_cells_history: VecDeque<f64>,
    waste_pct_history: VecDeque<f64>,
    efficiency_history: VecDeque<f64>,
    owner_mem_history: VecDeque<f64>,
    stall_history: VecDeque<f64>,
    disk_util_history: VecDeque<f64>,
...
impl App {
    pub fn new(db: TuiDb, build_version: String) -> Self {
        ...
        Self {
            ...
            us_per_cell_build_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            us_per_cell_flush_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            target_cells_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            waste_pct_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            efficiency_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            owner_mem_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            stall_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
            disk_util_history: VecDeque::with_capacity(RATE_HISTORY_SIZE),
    ...
    fn sample_rates(&mut self) {
            ...
            .and_then(|s| s.bulk_build.as_ref())
        {
            ...
            if batch_count > 0 && batch_count != self.last_overlap_batch_count {
                ...
                let cells_created = bb.cells_created.unwrap_or(0) as f64;
                if cells_created > 0.0 {
                    push_history_sample(
                        &mut self.us_per_cell_build_history,
                        build_ms * 1000.0 / cells_created,
                    );
                    let flush_ms_val = bb.flush_ms.unwrap_or(0.0);
                    push_history_sample(
                        &mut self.us_per_cell_flush_history,
                        flush_ms_val * 1000.0 / cells_created,
                    );
                }
                let iteration_ms = build_ms + prefetch_recv_ms + flush_wait_ms;
                if iteration_ms > 0.0 {
                    push_history_sample(
                        &mut self.efficiency_history,
                        build_ms / iteration_ms * 100.0,
                    );
                }
                let build_ema = bb.controller_build_ema.unwrap_or(0.0);
                if build_ema > 1.0 {
                    push_history_sample(
                        &mut self.stall_history,
                        if build_ms > build_ema * 2.0 { 1.0 } else { 0.0 },
                    );
                }
                ...
                if total_ema > 0.0 {
                    push_history_sample(
                        &mut self.waste_pct_history,
                        (recv_ema + wait_ema) / total_ema * 100.0,
                    );
                }
                push_history_sample(
                    &mut self.target_cells_history,
                    bb.target_cells.unwrap_or(0) as f64,
                );
                push_history_sample(
                    &mut self.owner_mem_history,
                    bb.owner_memory_bytes.unwrap_or(0) as f64,
                );
                push_history_sample(&mut self.disk_util_history, bb.disk_util_pct.unwrap_or(0.0));
...
fn draw_sync_content(f: &mut Frame, app: &App, area: Rect) {
    ...
    if is_bulk_build {
        ...
        draw_bulk_sync_dashboard(f, app, chunks[2]);
    } else {
...
const BEST_US_PER_CELL_BUILD: f64 = 6.6;
const BEST_US_PER_CELL_FLUSH: f64 = 13.4;

fn us_per_cell_color(value: f64, baseline: f64) -> Color {
    if value <= baseline {
        TERMINAL_GREEN
...
fn backpressure_label(flush_wait_ms: f64, build_ms: f64) -> (&'static str, Color) {
    if build_ms < 1.0 {
        return ("~balanced", SLATE_500);
    }
    let ratio = flush_wait_ms / build_ms;
    if ratio > 0.3 {
        ("\u{2190}pressure", ERROR_RED)
    } else if ratio <= 0.1 {
        ("\u{2192}flow", TERMINAL_GREEN)
    } else {
        ("~balanced", SLATE_500)
    }
}

fn dashboard_section_header(title: &str, width: u16) -> Line<'static> {
    let dashes = "\u{2500}".repeat(3);
    let trail_len = (width as usize).saturating_sub(title.len() + 8);
    let trail = "\u{2500}".repeat(trail_len);
    Line::from(vec![Span::styled(
        format!("{} {} {}", dashes, title, trail),
...
// ---------------------------------------------------------------------------
// Bulk Sync Dashboard — row group renderers
// ---------------------------------------------------------------------------

/// Pipeline row group: prefetch -> build -> flush -> disk flow.
fn dashboard_pipeline_lines(
    bb: &BulkBuildProgressData,
    app: &App,
    col_width: u16,
    ...
    let flush_wait_ms = bb.flush_wait_ms.unwrap_or(0.0);
    let fetch_ms = bb.fetch_ms.unwrap_or(0.0);
    let disk_util = bb.disk_util_pct.unwrap_or(0.0);
    let owner_mem = bb.owner_memory_bytes.unwrap_or(0);
    let live_cells = bb.live_cell_count.unwrap_or(0);
    let iteration_ms = build_ms + prefetch_recv_ms + flush_wait_ms;

    let mut left: Vec<Line<'static>> = Vec::new();
    let mut right: Vec<Line<'static>> = Vec::new();

    // Prefetch channel indicator
    let pf_label = match (bb.prefetch_channel_pending, bb.prefetch_channel_capacity) {
        (Some(p), Some(c)) if c > 0 => {
            let filled = ((p as f64 / c as f64) * 4.0).round() as usize;
            format!("{} {}/{}", render_gauge(filled.min(4), 4), p, c)
        }
        _ => "-".to_string(),
    };

    // Top 2 sub-phases by time
    let mut sub_phases: Vec<(&str, f64)> = vec![
        ("facts", bb.facts_ms.unwrap_or(0.0)),
        ("resolve", bb.resolve_ms.unwrap_or(0.0)),
        ("reduce", bb.reduce_ms.unwrap_or(0.0)),
        ("history", bb.history_ms.unwrap_or(0.0)),
        ("addr", bb.address_reduce_ms.unwrap_or(0.0)),
        ("activity", bb.activity_stats_ms.unwrap_or(0.0)),
    ];
    sub_phases.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    let top2: String = if bb.controller_bottleneck == Some(2) {
        sub_phases
            .iter()
            .take(2)
            .filter(|(_, ms)| *ms > 0.0)
            .map(|(name, ms)| format!("{} {:.0}%", name, ms / build_ms * 100.0))
            .collect::<Vec<_>>()
            .join(" ")
    } else {
        String::new()
    ...
    // Color build time by band: green if in [2s, 5s], red otherwise, SLATE_500 if near zero
    let build_time_color = if build_ms < 1.0 {
        SLATE_500
    } else if (2000.0..=5000.0).contains(&build_ms) {
        TERMINAL_GREEN
    } else {
        ERROR_RED
    };

    // Row 1: prefetch -> build Xs [top2]
    let mut row1_spans = vec![
        Span::styled("prefetch ", Style::default().fg(SLATE_500)),
        Span::styled(pf_label, Style::default().fg(TERMINAL_DIM)),
        Span::styled(" \u{2192} build ", Style::default().fg(FOREGROUND)),
        Span::styled(
            format!("{:.2}s", build_ms / 1000.0),
            Style::default().fg(build_time_color),
        ),
    ];
    if !top2.is_empty() {
    ...
    left.push(Line::from(row1_spans));

    // Flush channel indicator
    let fl_label = match (bb.flush_channel_pending, bb.flush_channel_capacity) {
        (Some(p), Some(c)) if c > 0 => {
            let fl_color = if p >= c {
                ERROR_RED
            } else if p as f64 >= c as f64 * 0.75 {
                AMBER
            } else {
                TERMINAL_DIM
            };
            (
                format!(
                    "{} {}/{}",
                    render_gauge(((p as f64 / c as f64) * 4.0).round() as usize, 4),
                    p,
                    c
                ),
                fl_color,
        ...
        _ => ("-".to_string(), SLATE_500),
    };

    let disk_color = if disk_util > 90.0 {
        ERROR_RED
    } else if disk_util > 70.0 {
        AMBER
    } else {
        TERMINAL_DIM
    };

    // Row 2: -> flush Xms [P/C] -> disk D%
    left.push(Line::from(vec![
        Span::styled(
            format!("\u{2192} flush {:.0}ms ", flush_ms),
            Style::default().fg(CYAN),
        ),
        Span::styled(fl_label.0, Style::default().fg(fl_label.1)),
        Span::styled(
            format!(" \u{2192} disk {:.0}%", disk_util),
            Style::default().fg(disk_color),
        ),
    ]));

    // Standard density: CPU + I/O overlap bars
    if density == DashboardDensity::Standard {
        let bar_width = (col_width as usize).saturating_sub(22).max(4);
        if iteration_ms > 0.0 && build_ms > 0.0 {
            let col = |ms: f64| -> usize {
                ((ms / iteration_ms * bar_width as f64).round() as usize).min(bar_width)
            };
            let eff = build_ms / iteration_ms * 100.0;
            let mem_str = format_bytes(owner_mem);

            // CPU row
            if app.show_build_subphases {
                let sub_phase_colors: &[(f64, Color)] = &[
                    (bb.facts_ms.unwrap_or(0.0), AMBER),
                    (bb.resolve_ms.unwrap_or(0.0), Color::Magenta),
                    (bb.reduce_ms.unwrap_or(0.0), Color::Blue),
                    (bb.history_ms.unwrap_or(0.0), TERMINAL_GREEN),
                ...
                let mut spans = vec![Span::styled("Build", Style::default().fg(AMBER))];
                let mut offset = 0.0;
                for &(dur, color) in sub_phase_colors {
                    let s = col(offset);
                    let e = col(offset + dur);
                    let len = e.saturating_sub(s);
                    if len > 0 {
            ...
            let mut io_spans = vec![Span::styled("I/O  ", Style::default().fg(SLATE_500))];
            if fetch_cols > 0 {
            ...
            let gap = flush_start.saturating_sub(fetch_cols);
            if gap > 0 {
                io_spans.push(Span::raw(" ".repeat(gap)));
            }
            if flush_len > 0 {
                io_spans.push(Span::styled(
                    "\u{2593}".repeat(flush_len),
            ...
            io_spans.push(Span::styled(
                format!(" live {}c", format_num_compact(live_cells)),
                Style::default().fg(SLATE_500),
            ));
            left.push(Line::from(io_spans));
        }
    } else {
        // Compact: single efficiency line
        if iteration_ms > 0.0 {
            let eff = build_ms / iteration_ms * 100.0;
            left.push(Line::from(vec![
                Span::styled(
                    format!("eff {:.0}%", eff),
                    Style::default().fg(if eff >= 70.0 { TERMINAL_GREEN } else { AMBER }),
                ),
                Span::styled(
                    format!(
                        "  mem {}  live {}c",
                        format_bytes(owner_mem),
                        format_num_compact(live_cells)
                    ),
                    Style::default().fg(SLATE_500),
                ),
            ]));
    ...
    let spark_w = (col_width as usize).saturating_sub(18).clamp(4, 20);
    right.push(Line::from(vec![
        Span::styled("build ", Style::default().fg(SLATE_500)),
        Span::styled(
            sparkline(&app.build_cpu_ms_history, spark_w),
            Style::default().fg(TERMINAL_GREEN),
        ),
    ]));
    right.push(Line::from(vec![
        ...
        Span::styled(
            sparkline(&app.flush_wait_ms_history, spark_w),
            Style::default().fg(CYAN),
        ),
    ]));
    if density == DashboardDensity::Standard {
        let (bp_label, bp_color) = backpressure_label(flush_wait_ms, build_ms);
        right.push(Line::from(vec![Span::styled(
            bp_label.to_string(),
            Style::default().fg(bp_color),
        )]));
        // pad right to match left
...
/// Throughput row group: block/s, tx/s, us/cell metrics.
fn dashboard_throughput_lines(
    bb: &BulkBuildProgressData,
    app: &App,
    col_width: u16,
    density: DashboardDensity,
    ...
    let tx_rate = app
        ...
        .and_then(|s| s.tx_rate_realtime)
        .unwrap_or(0.0);
    let batch_blocks = bb.batch_block_count.unwrap_or(0);
    let build_ms = bb.build_ms.unwrap_or(0.0);
    let flush_ms_val = bb.flush_ms.unwrap_or(0.0);
    ...
    left.push(Line::from(vec![
        Span::styled(
            format!("{:.1}K blk/s", block_rate / 1000.0),
            Style::default().fg(FOREGROUND),
        ),
        Span::styled(
            format!("  {:.0}K tx/s", tx_rate / 1000.0),
            Style::default().fg(SLATE_500),
        ),
        Span::styled(
            format!("  {}K blk/bat", batch_blocks / 1000),
            Style::default().fg(SLATE_500),
        ),
    ]));

    // Row 2: us/cell
    let us_build = if cells_created > 0 {
        build_ms * 1000.0 / cells_created as f64
    } else {
        0.0
    };
    let us_flush = if cells_created > 0 {
        flush_ms_val * 1000.0 / cells_created as f64
    } else {
        0.0
    };
    left.push(Line::from(vec![
        Span::styled("us/cell: build ", Style::default().fg(SLATE_500)),
        Span::styled(
            format!("{:.1}", us_build),
            Style::default().fg(us_per_cell_color(us_build, BEST_US_PER_CELL_BUILD)),
        ),
        Span::styled(" flush ", Style::default().fg(SLATE_500)),
        Span::styled(
            format!("{:.1}", us_flush),
            Style::default().fg(us_per_cell_color(us_flush, BEST_US_PER_CELL_FLUSH)),
        ),
    ]));

    // Row 3 (Standard): stall count + baseline
    if density == DashboardDensity::Standard {
        let recent_stalls: f64 = app.stall_history.iter().rev().take(60).sum();
        let total_stalls: f64 = app.stall_history.iter().sum();
        left.push(Line::from(vec![
            Span::styled(
                format!("stall {:.0}/min", recent_stalls),
                Style::default().fg(if recent_stalls > 2.0 {
                    AMBER
                } else {
                    SLATE_500
                }),
            ...
            Span::styled(
                format!(" ({:.0} total)", total_stalls),
                Style::default().fg(SLATE_500),
            ),
            Span::styled(
                format!(
                    "  best: {:.1}/{:.1}",
                    BEST_US_PER_CELL_BUILD, BEST_US_PER_CELL_FLUSH
                ),
                Style::default().fg(SLATE_700),
            ),
        ]));
    ...
    right.push(Line::from(vec![
        ...
        Span::styled(
            sparkline(&app.rate_history, spark_w),
            Style::default().fg(TERMINAL_GREEN),
        ),
    ]));
    right.push(Line::from(vec![
        Span::styled("us/cl ", Style::default().fg(SLATE_500)),
        Span::styled(
            sparkline(&app.us_per_cell_build_history, spark_w),
            Style::default().fg(AMBER),
        ),
    ]));
    if density == DashboardDensity::Standard {
        right.push(Line::from(vec![
            ...
            Span::styled(
                sparkline(&app.stall_history, spark_w),
                Style::default().fg(ERROR_RED),
            ),
        ]));
    }
...
/// Controller row group: bottleneck, band, sizing, knobs.
fn dashboard_controller_lines(
    bb: &BulkBuildProgressData,
    app: &App,
    col_width: u16,
    density: DashboardDensity,
    ...
    let band_position = ((build_ema - 2000.0) / 3000.0 * 8.0)
        .round()
        .clamp(0.0, 8.0) as usize;
    let band_bar = render_gauge(band_position, 8);

    // Sizing decision
    let sizing: (&str, Color) = if let Some(d) = deltas {
        let pct = d.budget_delta_pct;
        if pct > 0.5 {
            ("GROW", TERMINAL_GREEN)
        } else if pct < -0.5 {
    ...
    // Raw deviation
    let raw_dev: String = if build_ema > 1.0 {
        let dev = ((build_ms - build_ema) / build_ema).abs();
        if dev >= 0.1 {
            format!(" (raw {:.1})", build_ms / 1000.0)
        } else {
            String::new()
        }
    } else {
    ...
    let delta_pct_text: String = deltas
        .map(|d| {
            let pct = d.budget_delta_pct;
            if pct.abs() < 0.5 {
                String::new()
    ...
    // Row 1: [BOTTLENECK] band bar EMA (raw) SIZING +X%
    left.push(Line::from(vec![
        Span::styled(
            format!("[{}]", bn_label),
            Style::default().fg(bn_color).add_modifier(Modifier::BOLD),
        ...
        Span::styled(band_bar, Style::default().fg(build_band_color)),
        Span::styled(
            format!(" {:.2}s{}", build_ema / 1000.0, raw_dev),
            Style::default().fg(FOREGROUND),
        ),
        Span::styled(
            format!(" {}{}", sizing.0, delta_pct_text),
            Style::default().fg(sizing.1),
        ),
    ]));

    // Row 2: target cells fill% thr bg L0
    let budget_k = target_cells / 1_000;
    let fill_text = match (bb.facts_cell_count, bb.target_cells) {
        (Some(actual), Some(target)) if target > 0 => {
            let fill_pct = (actual as f64 / target as f64 * 100.0).min(999.0);
            ...
            (format!("fill {:.0}%", fill_pct), fill_color)
        }
        _ => ("-".to_string(), SLATE_500),
    };
    ...
    // Supply-cap badge
    let supply_cap = matches!(
        (bb.target_cells, bb.facts_cell_count),
        (Some(target), Some(actual)) if actual > 0 && target > actual * 4
    );
    ...
    let mut row2_spans = vec![
        ...
        Span::styled(fill_text.0, Style::default().fg(fill_text.1)),
    ];
    if supply_cap {
        row2_spans.push(Span::styled(" \u{26A1}capped", Style::default().fg(AMBER)));
    }
    row2_spans.extend([
        Span::styled(" thr ", Style::default().fg(SLATE_500)),
    ...
    left.push(Line::from(row2_spans));

    // Row 3 (Standard): waste composition
    if density == DashboardDensity::Standard {
        let total_io = recv_ema + wait_ema;
        if total_io > 1.0 {
            let recv_pct = recv_ema / total_io * 100.0;
            let wait_pct = wait_ema / total_io * 100.0;
            left.push(Line::from(vec![
                Span::styled("waste: recv ", Style::default().fg(SLATE_500)),
                Span::styled(format!("{:.0}%", recv_pct), Style::default().fg(AMBER)),
                Span::styled(" | flush ", Style::default().fg(SLATE_500)),
                Span::styled(format!("{:.0}%", wait_pct), Style::default().fg(ERROR_RED)),
            ]));
        } else {
    ...
    let spark_w = (col_width as usize).saturating_sub(18).clamp(4, 20);
    right.push(Line::from(vec![
        Span::styled("tgt   ", Style::default().fg(SLATE_500)),
        Span::styled(
            sparkline(&app.target_cells_history, spark_w),
            Style::default().fg(TERMINAL_GREEN),
        ),
    ]));
    right.push(Line::from(vec![
        Span::styled("L0    ", Style::default().fg(SLATE_500)),
        Span::styled(
            sparkline(&app.l0_files_history, spark_w),
            Style::default().fg(AMBER),
        ),
    ]));
    if density == DashboardDensity::Standard {
        right.push(Line::from(vec![
            ...
            Span::styled(
                sparkline(&app.waste_pct_history, spark_w),
                Style::default().fg(ERROR_RED),
            ),
        ]));
    }
...
/// Resources row group: WBM, disk I/O.
fn dashboard_resources_lines(
    bb: &BulkBuildProgressData,
    ...
    // Row 1: WBM usage + immutable memtables + L0
    if let Some(m) = mem {
        let wbm_pct = if m.wbm_budget_bytes > 0 {
            m.wbm_usage_bytes as f64 / m.wbm_budget_bytes as f64 * 100.0
        } else {
            0.0
        };
        let wbm_color = if wbm_pct > 90.0 {
            ERROR_RED
        } else if wbm_pct > 70.0 {
            AMBER
        } else {
            FOREGROUND
        };
        left.push(Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(wbm_color),
            ),
            Span::styled(
                format!(
                    " imm {} L0 {}/{}",
                    m.immutable_memtables, m.l0_files_max, L0_GAUGE_MAX
                ),
                Style::default().fg(SLATE_500),
            ),
        ]));
    ...
    // Row 2: Disk I/O
    let disk_util = bb.disk_util_pct.unwrap_or(0.0);
    let disk_wr = bb.disk_write_mb_s.unwrap_or(0.0);
    let disk_qd = bb.disk_avg_queue_depth.unwrap_or(0.0);
    let disk_await = bb.disk_await_ms.unwrap_or(0.0);
    let disk_color = if disk_util > 90.0 {
        ERROR_RED
    } else if disk_util > 70.0 {
        AMBER
    ...
    left.push(Line::from(vec![
        ...
        Span::styled(
            format!(
                " wr {:.0}M/s qd {:.1} await {:.0}ms",
                disk_wr, disk_qd, disk_await
            ),
            Style::default().fg(SLATE_500),
        ),
    ]));

    // Right: disk sparkline
    let spark_w = (col_width as usize).saturating_sub(18).clamp(4, 20);
    right.push(Line::from(vec![
        Span::styled("disk  ", Style::default().fg(SLATE_500)),
        Span::styled(
            sparkline(&app.disk_util_history, spark_w),
            Style::default().fg(AMBER),
        ),
    ]));
    right.push(Line::from(""));
...
/// Build stages row group: proportional bars for 6 build sub-phases.
fn dashboard_build_stages_lines(bb: &BulkBuildProgressData, full_width: u16) -> Vec<Line<'static>> {
    let stages: [(&str, f64, Color); 6] = [
        ("facts", bb.facts_ms.unwrap_or(0.0), AMBER),
        ("resolve", bb.resolve_ms.unwrap_or(0.0), Color::Magenta),
        ("reduce", bb.reduce_ms.unwrap_or(0.0), Color::Blue),
        ("addr", bb.address_reduce_ms.unwrap_or(0.0), CYAN),
        ("history", bb.history_ms.unwrap_or(0.0), TERMINAL_GREEN),
        ("activity", bb.activity_stats_ms.unwrap_or(0.0), FOREGROUND),
    ];

    let total_ms: f64 = stages.iter().map(|(_, ms, _)| ms).sum();
    if total_ms < 1.0 {
        return vec![
            Line::from(Span::styled(
                "(no stage data)",
    ...
    let usable_width = (full_width as usize).saturating_sub(4);
    // Each stage: "name ██░ Xms X%  " -> ~20 chars; 3 per row
    let per_stage_width = usable_width / 3;

    let render_stage = |name: &str, ms: f64, color: Color| -> Vec<Span<'static>> {
        let pct = ms / total_ms * 100.0;
        let bar_chars = (per_stage_width).saturating_sub(name.len() + 12).max(1);
        let filled = ((pct / 100.0) * bar_chars as f64).round() as usize;
        let bar = render_gauge(filled.min(bar_chars), bar_chars);
        vec![
            Span::styled(format!("{} ", name), Style::default().fg(SLATE_500)),
    ...
    // Row 1: facts, resolve, reduce
    let mut row1_spans: Vec<Span<'static>> = Vec::new();
    for &(name, ms, color) in stages.iter().take(3) {
        row1_spans.extend(render_stage(name, ms, color));
    }
    // Row 2: addr, history, activity
    let mut row2_spans: Vec<Span<'static>> = Vec::new();
    for &(name, ms, color) in stages.iter().skip(3) {
        row2_spans.extend(render_stage(name, ms, color));
    }

    vec![Line::from(row1_spans), Line::from(row2_spans)]
}

/// Ultra-compact: 2-line summary for very small terminals.
fn dashboard_ultra_compact_lines(
    bb: &BulkBuildProgressData,
    stall_history: &VecDeque<f64>,
    rate_history: &VecDeque<f64>,
) -> Vec<Line<'static>> {
    let build_ms = bb.build_ms.unwrap_or(0.0);
    let cells_created = bb.cells_created.unwrap_or(0);
    let block_rate = rate_history.back().copied().unwrap_or(0.0);
    ...
    let recent_stalls: f64 = stall_history.iter().rev().take(60).sum();

    // Row 1: build time, blk/s, us/cell, stall
    let row1 = Line::from(vec![
        Span::styled(
            format!("build {:.2}s", build_ms / 1000.0),
            Style::default().fg(FOREGROUND),
        ),
        Span::styled(
            format!("  {:.1}K blk/s", block_rate / 1000.0),
        ...
        Span::styled(
            format!("  us/cell {:.1}/{:.1}", us_build, us_flush),
            Style::default().fg(SLATE_500),
        ),
        Span::styled(
            format!("  stall {:.0}/min", recent_stalls),
    ...
    // Row 2: bottleneck, target, knobs, L0, flush channel, disk
    let (bn_label, bn_color) = match bb.controller_bottleneck {
        Some(1) => ("FETCH", AMBER),
        Some(2) => ("BUILD", TERMINAL_GREEN),
        Some(3) => ("FLUSH", ERROR_RED),
    ...
    let target_k = bb.target_cells.unwrap_or(0) / 1_000;
    let threads = bb
        .controller_fetch_threads
        .map(|v| v.to_string())
        .unwrap_or_else(|| "-".to_string());
    let bg = bb
        ...
        .unwrap_or_else(|| "-".to_string());
    let l0_ema = bb.controller_l0_ema.unwrap_or(0.0);
    let fl_text = match (bb.flush_channel_pending, bb.flush_channel_capacity) {
    ...
    let row2 = Line::from(vec![
        ...
        Span::styled(
            format!(
                " {}K thr {} bg {} L0 {:.0} fl {} Disk {:.0}%",
                target_k, threads, bg, l0_ema, fl_text, disk_util
            ),
            Style::default().fg(SLATE_500),
        ),
    ]);
...
// ---------------------------------------------------------------------------
// Bulk Sync Dashboard — assembler
// ---------------------------------------------------------------------------

fn draw_bulk_sync_dashboard(f: &mut Frame, app: &App, area: Rect) {
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(SLATE_800))
        .title(Span::styled(
    ...
    let Some(sync) = &app.sync_status else {
        f.render_widget(Paragraph::new("No bulk-build data"), inner);
        return;
    };
    let Some(bb) = sync.bulk_build.as_ref() else {
        f.render_widget(Paragraph::new("No bulk-build data"), inner);
        return;
    };

    // Finalize phase: delegate to existing renderer
    if bb.finalize_phase.is_some() {
        let lines = build_finalize_left_column(bb, inner.height as usize);
        f.render_widget(Paragraph::new(lines), inner);
        return;
    }

    let density = dashboard_density(inner.height, app.diagnostics_view_mode);

    // Ultra-compact: just 2 lines
    if density == DashboardDensity::UltraCompact {
        let lines = dashboard_ultra_compact_lines(bb, &app.stall_history, &app.rate_history);
        f.render_widget(Paragraph::new(lines), inner);
        return;
    }

    // Standard/Compact: 2-column layout
    let col_split = Layout::default()
        .direction(Direction::Horizontal)
        .constraints([Constraint::Percentage(55), Constraint::Percentage(45)])
        .split(inner);

    let col_w_left = col_split[0].width;
    let col_w_right = col_split[1].width;

    let mut all_left: Vec<Line<'static>> = Vec::new();
    let mut all_right: Vec<Line<'static>> = Vec::new();

    // Pipeline section
    all_left.push(dashboard_section_header("Pipeline", col_w_left));
    all_right.push(Line::from(""));
    let (pl, pr) = dashboard_pipeline_lines(bb, app, col_w_right, density);
    all_left.extend(pl);
    all_right.extend(pr);

    // Throughput section
    all_left.push(dashboard_section_header("Throughput", col_w_left));
    all_right.push(Line::from(""));
    let (tl, tr) = dashboard_throughput_lines(bb, app, col_w_right, density);
    all_left.extend(tl);
    all_right.extend(tr);

    // Controller section
    all_left.push(dashboard_section_header("Controller", col_w_left));
    all_right.push(Line::from(""));
    let (cl, cr) = dashboard_controller_lines(
        bb,
        app,
        col_w_right,
        density,
    ...
    // Standard: Resources or Build Stages (toggle via show_build_subphases)
    if density == DashboardDensity::Standard {
        if app.show_build_subphases {
            all_left.push(dashboard_section_header(
                "Build Stages (e to close)",
                col_w_left,
            ));
            all_right.push(Line::from(""));
            let stage_lines = dashboard_build_stages_lines(bb, col_w_left);
            // Stages span both columns - put in left, blank right
            all_left.extend(stage_lines);
            all_right.push(Line::from(""));
            all_right.push(Line::from(""));
        } else {
            all_left.push(dashboard_section_header("Resources", col_w_left));
            all_right.push(Line::from(""));
            let (rl, rr) =
                dashboard_resources_lines(bb, app.memory_stats.as_ref(), app, col_w_right);
            all_left.extend(rl);
            all_right.extend(rr);
        }
    }

    // Background tasks: single inline row if any
    let visible_refs = visible_background_tasks(&app.background_tasks);
    if !visible_refs.is_empty() {
        let mut task_spans: Vec<Span<'static>> =
            vec![Span::styled("bg: ", Style::default().fg(SLATE_700))];
        for (i, t) in visible_refs.iter().take(3).enumerate() {
            if i > 0 {
                task_spans.push(Span::styled("  ", Style::default().fg(SLATE_500)));
            }
            let name = background_task_display_name(t);
            let state = background_task_state_label(t);
            let color = background_task_state_color(t);
            task_spans.push(Span::styled(
                format!("{}: {}", name, state),
                Style::default().fg(color),
            ));
        }
        all_left.push(Line::from(task_spans));
        all_right.push(Line::from(""));
    }

    // Pad columns to equal length
    while all_right.len() < all_left.len() {
        all_right.push(Line::from(""));
    }
    while all_left.len() < all_right.len() {
        all_left.push(Line::from(""));
    }

    f.render_widget(Paragraph::new(all_left), col_split[0]);
    f.render_widget(Paragraph::new(all_right), col_split[1]);
}
...
fn dashboard_density(available_rows: u16, mode: DiagnosticsViewMode) -> DashboardDensity {
    match mode {
        DiagnosticsViewMode::Compact => {
            if available_rows < 10 {
                DashboardDensity::UltraCompact
            } else {
                DashboardDensity::Compact
            }
        }
        DiagnosticsViewMode::Detail => DashboardDensity::Standard,
        DiagnosticsViewMode::Auto => {
            if available_rows >= 21 {
                DashboardDensity::Standard
            } else if available_rows >= 10 {
                DashboardDensity::Compact
            } else {
                DashboardDensity::UltraCompact
            }
...
mod tests {
    use super::{
        ...
        backpressure_label, build_finalize_left_column, bulk_queue_indicator_line,
        chart_height_warning, compact_overview_layout, consumed_cells_source_color,
        consumed_cells_source_label, controller_panel_lines, dashboard_build_stages_lines,
        dashboard_density, dashboard_ultra_compact_lines, dense_right_lines, detail_right_lines,
        ...
        trend_delta, trim_for_panel, us_per_cell_color, visible_background_tasks, App, Color,
        CompactOverviewLayout, ControllerDeltas, DashboardDensity, DiagnosticsViewMode,
        SyncBottleneck, AMBER, BEST_US_PER_CELL_BUILD, CYAN, ERROR_RED, STATUS_MESSAGE_TTL_SECS,
        TERMINAL_DIM, TERMINAL_GREEN,
    };
    use crate::db::{
    ...
    #[test]
    fn test_dashboard_density_detection() {
        assert_eq!(
            dashboard_density(25, DiagnosticsViewMode::Auto),
            DashboardDensity::Standard
        );
        assert_eq!(
            dashboard_density(21, DiagnosticsViewMode::Auto),
            DashboardDensity::Standard
        );
        assert_eq!(
            dashboard_density(20, DiagnosticsViewMode::Auto),
            DashboardDensity::Compact
        );
        assert_eq!(
            dashboard_density(10, DiagnosticsViewMode::Auto),
            DashboardDensity::Compact
        );
        assert_eq!(
            dashboard_density(9, DiagnosticsViewMode::Auto),
            DashboardDensity::UltraCompact
        );
        assert_eq!(
            dashboard_density(5, DiagnosticsViewMode::Auto),
            DashboardDensity::UltraCompact
        );
        assert_eq!(
            dashboard_density(30, DiagnosticsViewMode::Compact),
            DashboardDensity::Compact
        );
        assert_eq!(
            dashboard_density(30, DiagnosticsViewMode::Detail),
            DashboardDensity::Standard
        );
    }

    #[test]
    fn test_backpressure_label() {
        let (label, _) = backpressure_label(400.0, 1000.0);
        assert_eq!(label, "\u{2190}pressure");
        let (label, _) = backpressure_label(50.0, 1000.0);
        assert_eq!(label, "\u{2192}flow");
        let (label, _) = backpressure_label(200.0, 1000.0);
        assert_eq!(label, "~balanced");
        let (label, _) = backpressure_label(100.0, 0.5);
        assert_eq!(label, "~balanced");
    }

    #[test]
    fn test_us_per_cell_color() {
        assert_eq!(
            us_per_cell_color(5.0, BEST_US_PER_CELL_BUILD),
            TERMINAL_GREEN
        );
        assert_eq!(
            us_per_cell_color(BEST_US_PER_CELL_BUILD, BEST_US_PER_CELL_BUILD),
            TERMINAL_GREEN
        );
        assert_eq!(us_per_cell_color(8.0, BEST_US_PER_CELL_BUILD), AMBER);
        assert_eq!(us_per_cell_color(10.0, BEST_US_PER_CELL_BUILD), ERROR_RED);
    }

    #[test]
    fn test_dashboard_ultra_compact_lines() {
        let bb = BulkBuildProgressData {
            build_ms: Some(1940.0),
            flush_ms: Some(310.0),
            cells_created: Some(200_000),
            controller_bottleneck: Some(2),
            controller_build_ema: Some(2100.0),
            controller_recv_ema: Some(100.0),
            controller_wait_ema: Some(200.0),
            controller_l0_ema: Some(27.0),
            controller_fetch_threads: Some(8),
            controller_bg_jobs: Some(4),
            target_cells: Some(200_000),
            flush_channel_pending: Some(1),
            flush_channel_capacity: Some(2),
            disk_util_pct: Some(64.0),
            ..Default::default()
        };
        let lines = dashboard_ultra_compact_lines(&bb, &VecDeque::new(), &VecDeque::new());
        assert_eq!(lines.len(), 2, "ultra-compact should have 2 lines");
        let r1: String = lines[0].spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(
            r1.contains("build") && r1.contains("blk/s"),
            "row 1: {}",
            r1
        );
        let r2: String = lines[1].spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(r2.contains("[BUILD]") && r2.contains("L0"), "row 2: {}", r2);
    }

    #[test]
    fn test_dashboard_build_stages_lines() {
        let bb = BulkBuildProgressData {
            facts_ms: Some(175.0),
            resolve_ms: Some(86.0),
            reduce_ms: Some(520.0),
            history_ms: Some(328.0),
            address_reduce_ms: Some(438.0),
            activity_stats_ms: Some(94.0),
            build_ms: Some(1641.0),
            ..Default::default()
        };
        let lines = dashboard_build_stages_lines(&bb, 80);
        assert_eq!(lines.len(), 2, "build stages should have 2 lines");
        let r1: String = lines[0].spans.iter().map(|s| s.content.as_ref()).collect();
        assert!(
            r1.contains("facts") && r1.contains("resolve") && r1.contains("reduce"),
            "row 1: {}",
            r1
        );
    }
