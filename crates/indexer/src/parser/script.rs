...
impl ScriptParser {
    ...
    pub fn compute_script_hash(script: &Script) -> Result<Vec<u8>> {
    ...
    pub fn parse_hash_type(hash_type: &str) -> Result<u8> {
        match hash_type {
            "data" => Ok(0),
            "type" => Ok(1),
            "data1" => Ok(2),
            "data2" => Ok(4),
            other => Err(anyhow!("unknown hash_type: '{}'", other)),
        }
    }

    pub fn hash_type_to_i16(hash_type: &str) -> Result<i16> {
        Ok(Self::parse_hash_type(hash_type)? as i16)
    }
}
...
mod tests {
    ...
    fn test_secp256k1_blake160_type_script_hash() {
        ...
        let hash = ScriptParser::compute_script_hash(&script).unwrap();
    ...
    #[test]
    fn test_parse_hash_type_known_types() {
    ...
    #[test]
    fn test_parse_hash_type_unknown_errors() {
        let err = ScriptParser::parse_hash_type("unknown").unwrap_err();
        assert!(err.to_string().contains("unknown hash_type"));
    }
}
