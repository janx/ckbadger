...
impl TransactionParser {
    pub fn parse(tx: &TransactionView) -> Result<ParsedTransaction> {
        ...
        let hash = Self::parse_hex_hash32(&tx.hash);
        let version = Self::parse_hex_u32(&tx.version);
        ...
        tx.inputs
            ...
            .map(|(input_idx, input)| {
                let previous_tx_hash =
                    Self::parse_hex_hash32(&input.previous_output.tx_hash);
                let previous_output_index_u32 =
                    Self::parse_hex_u32(&input.previous_output.index);
    ...
    pub fn parse_cell_deps(tx: &TransactionView) -> Result<Vec<ParsedCellDep>> {
        tx.cell_deps
            ...
            .map(|(dep_idx, cell_dep)| {
                let out_point_tx_hash = Self::parse_hex_hash32(&cell_dep.out_point.tx_hash);
                let out_point_index_u32 = Self::parse_hex_u32(&cell_dep.out_point.index);
                ...
                Ok(ParsedCellDep {
                    ...
                    dep_type: match cell_dep.dep_type.as_str() {
                        "code" => 0,
                        "dep_group" => 1,
                        _ => unreachable!("node-validated dep_type: '{}'", cell_dep.dep_type),
                    },
                })
            })
            .collect()
    ...
    fn parse_since(since_hex: &str) -> u64 {
        let raw = since_hex;
...
mod tests {
    ...
    fn test_parse_since_absolute() {
        ...
        assert_eq!(since, 0x7fffffffffffffff);
    }
    ...
    #[test]
    fn test_parse_since_high_bit_allowed() {
        let since_hex = "0xa00000000000000c";
        let since = TransactionParser::parse_since(since_hex);
        assert_eq!(since, 0xa00000000000000c);
    }
