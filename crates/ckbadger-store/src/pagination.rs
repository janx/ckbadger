...
impl CkbadgerStore {
    ...
    /// Reverse iterate a CF from a starting key, returning up to `limit` items.
    pub fn paginate_reverse<T, F>(
        &self,
        cf: &ColumnFamily,
        from_key: &[u8],
        prefix: &[u8],
    ...
    where
        F: Fn(&[u8], &[u8]) -> Option<T>,
    {
        let start_key = cursor.map(|c| c.last_key.as_slice()).unwrap_or(from_key);
