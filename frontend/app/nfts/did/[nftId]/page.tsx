...
export default function DidCkbItemDetailPage() {
  ...
  const detailQuery = useQuery({
    queryKey: ['did-ckb-item-detail', nftId],
    queryFn: () => api.getDidCkbItemDetail(nftId),
    retry: false,
  });
  ...
  const { data: itemActivities, isLoading: isActivitiesLoading } = useQuery({
    queryKey: ['did-ckb-item-activities', detail?.nftId, activityCursor],
    queryFn: () => {
      const queryParams: { limit: number; cursor?: string } = { limit: 20 };
      if (activityCursor) {
        queryParams.cursor = activityCursor;
      }
      return api.getDidCkbItemActivities(detail!.nftId, queryParams);
    },
        ...
        <main className="container mx-auto px-4 py-8">
          <TerminalPanel>
            <TerminalPanelContent className="py-12 text-center">
              <h2 className="text-xl text-slate-400">did:ckb item not found</h2>
            </TerminalPanelContent>
          </TerminalPanel>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <PageHeader
          title={detail.name || 'did:ckb identity'}
          badge={
            detail.isLive ? (
        ...
        <div className="space-y-6">
            ...
            <TerminalPanelContent>
              <DataGrid columns={2}>
                <DataField label="Standard">
                  <span className="font-mono text-slate-200">DID:CKB</span>
                </DataField>
          ...
          <TerminalPanel>
            ...
            <TerminalPanelContent>
              <DataGrid columns={1}>
                <DataField label="did:ckb Name">
                  <span className="font-mono text-slate-200">{detail.name || 'Unavailable'}</span>
                </DataField>
                <DataField label="DID ID" layout="vertical" valueClassName="w-full">
            ...
            <TerminalPanelContent>
              {detail.isLive && liveTxHash != null && liveOutputIndex != null ? (
                ...
                <div className="font-mono text-sm text-slate-400">
                  Recycled did:ckb identity has no live cell.
                </div>
              )}
            </TerminalPanelContent>
          </TerminalPanel>
